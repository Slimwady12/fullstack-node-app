...
async function streamOpenAI({ messages, temperature = 0.7, userId, onToken }) {
  ...
  console.log(`[openaiProxy] Streaming OpenAI for user ${userId}, model: ${FORCED_MODEL}`);
  ...
  const latency = Date.now() - startTime;
  console.log(`[openaiProxy] Stream complete for user ${userId}, latency: ${latency}ms, length: ${fullContent.length}`);

  return {
    content: fullContent,
    tokens: { input: promptTokens, output: fullContent.split(/\s+/).length },
    model: FORCED_MODEL,
    latency,
