...
const NAV_ITEMS: NavItem[] = [
  { key: 'dashboard', path: '/user', icon: <LayoutDashboard className="w-5 h-5" />, labelKey: 'common.navigation.dashboard' },
  { key: 'chat', path: '/user/chat', icon: <MessageSquare className="w-5 h-5" />, labelKey: 'common.navigation.chat' },
  { key: 'documents', path: '/user/documents', icon: <FileText className="w-5 h-5" />, labelKey: 'common.navigation.documents' },
  { key: 'lawyers', path: '/user/lawyers', icon: <Scale className="w-5 h-5" />, labelKey: 'common.navigation.lawyers' },
  { key: 'jobs', path: '/user/jobs', icon: <Briefcase className="w-5 h-5" />, labelKey: 'common.navigation.jobs' },
  { key: 'messages', path: '/user/messages', icon: <Mail className="w-5 h-5" />, labelKey: 'common.navigation.messages' },
  { key: 'settings', path: '/user/settings', icon: <Settings className="w-5 h-5" />, labelKey: 'common.navigation.settings' },
];
...
export default function UserLayout(): JSX.Element {
  ...
  return (
    <div className="min-h-screen flex bg-navy-950" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-64 bg-navy-900 border-r border-navy-800 fixed top-0 left-0 bottom-0 z-30" role="navigation" aria-label="User Navigation">
      ...
      <div className="flex-1 md:ml-64 flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-semibold text-lg">
                  {NAV_ITEMS.find(item => isActive(item.path)) ? t(NAV_ITEMS.find(item => isActive(item.path))!.labelKey as any) : t('user.dashboard.title' as any)}
                </h2>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-3 pl-2 border-l border-navy-700 ml-1">
                <div className="hidden sm:flex flex-col items-end">
                  ...
                  <span className="text-slate-500 text-xs">{user.phone.replace(/(\+998)(\d{2})(\d{3})(\d{2})(\d{2})/, '$1 ($2) $3-$4-$5')}</span>
                </div>
                <div className="w-10 h-10 rounded-full bg-emerald-600/20 flex items-center justify-center flex-shrink-0">
                  <UserCircle className="w-6 h-6 text-emerald-500" />
                </div>
        ...
        {/* Content */}
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 pb-24 md:pb-8" style={{ paddingBottom: 'max(2rem, env(safe-area-inset-bottom))' }}>
          <ErrorBoundary>
            <Outlet />
          </ErrorBoundary>
        </main>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-navy-900/95 backdrop-blur-sm border-t border-navy-800 z-20" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }} role="navigation" aria-label="Mobile Navigation">
          <div className="flex items-center justify-around px-2 py-1">
            {BOTTOM_NAV_ITEMS.map(item => (
              <Link
                key={item.key}
                to={item.path}
                className={`flex flex-col items-center justify-center py-2 px-3 min-h-[56px] flex-1 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-emerald-500 rounded-lg ${
                  isActive(item.path)
                    ? 'text-emerald-400'
              ...
              >
                ...
                <span className="text-[10px] mt-0.5 font-medium truncate">{t(item.labelKey as any)}</span>
              </Link>
            ))}
          </div>
        </nav>
